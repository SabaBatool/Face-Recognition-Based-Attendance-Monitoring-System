...
def getImagesAndLabels(path):
    # Lists to hold images and corresponding IDs
    faces = []
    IDs = []
    ...
    # Loop through all the images in the specified path
    for imagePath in os.listdir(path):
        # Construct full image path
        fullPath = os.path.join(path, imagePath)

        # Read the image
        gray_image = cv2.imread(fullPath, cv2.IMREAD_GRAYSCALE)

        # Ensure the image was read correctly
        if gray_image is None:
            print(f"Warning: Unable to read image {fullPath}. Skipping.")
            continue

        # Attempt to extract the ID from the filename
        try:
            ...
            print(f"Error processing file {imagePath}: {e}. Ensure it follows the format 'name.ID.sampleNumber.jpg'.")
...
def TrackImages():
    ...
    # Track recognized IDs to prevent duplicate attendance entries
    marked_ids = set()
    ...
    try:
        while True:
            ...
            for (x, y, w, h) in faces:
                ...
                print(f'Serial: {serial}, Confidence: {conf}')  # Debugging output

                if conf < 50 and serial not in marked_ids:  # Recognize and mark attendance if not already marked
                    ts = time.time()
                    date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
                    timeStamp = datetime.datetime.fromtimestamp(ts).strftime('%H:%M:%S')

                    # Retrieve name and ID
                    aa = df.loc[df['SERIAL NO.'] == serial]['NAME']
                    ID = df.loc[df['SERIAL NO.'] == serial]['ID']

                    if aa.empty or ID.empty:
                        print("No matching record found in the database.")
                        continue  # Skip if no matching record

                    ID = str(ID.values[0])  # Get the first ID value
                    bb = str(aa.values[0])  # Get the first name value

                    print(f'Attendance Entry: ID={ID}, Name={bb}, Date={date}, Time={timeStamp}')  # Debugging output

                    # Mark attendance by writing to CSV
                    ...
                    with open(attendance_file, 'a+', newline='') as csvFile1:
                        ...
                        if os.stat(attendance_file).st_size == 0:
                            ...
                            writer.writerow([ID, bb, date, timeStamp])  # Write current attendance
                          # Write current attendance
                    
                    tv.insert('', 'end', values=(ID, bb, date, timeStamp))


                    print("Attendance marked successfully.")  # Debugging output
                    marked_ids.add(serial)  # Add ID to marked list to avoid re-marking

                    # Show name on frame
                    # font = cv2.FONT_HERSHEY_SIMPLEX

                    cv2.putText(im, f' {bb}', (x, y + h + 30), font, 1, (0, 255, 0), 2)
                    print(f"//////////////////////Coordinates for text: ({x}, {y + h + 30})")  # Check the values

                    cv2.imshow('Taking Attendance', im)  # Show final image with marked attendance
                    cv2.waitKey(2000)  # Show for 2 seconds before closing
                    break  # Exit the face detection loop to close the camera
            ...
            # Break if all unique faces are marked or 'q' is pressed
            if len(marked_ids) >= len(df) or cv2.waitKey(1) == ord('q'):
                break
...
# Setup the Treeview widget in the frame
tv = ttk.Treeview(frame1, height=13, columns=('ID', 'name', 'date', 'time'))

# Configure the columns
tv.column('#0', width=0, stretch=tk.NO)  # Hide the #0 column if not needed
tv.column('ID', width=82)  # ID column definition
tv.column('name', width=130)
tv.column('date', width=133)
tv.column('time', width=133)

# Set up the Treeview grid layout
tv.grid(row=2, column=0, padx=(0, 0), pady=(150, 0), columnspan=4)

# Setup column headings
tv.heading('ID', text='ID')   # Heading for the ID column
...
trackImg = tk.Button(frame1, text="Take Attendance", command=TrackImages  ,fg="green"  ,bg="#3ffc00"  ,width=35  ,height=1, activebackground = "white" ,font=('comic', 15, ' bold '))
trackImg.place(x=30,y=50)
quitWindow = tk.Button(frame1, text="Quit", command=window.destroy  ,fg="black"  ,bg="#eb4600"  ,width=35 ,height=1, activebackground = "green" ,font=('comic', 15, ' bold '))
